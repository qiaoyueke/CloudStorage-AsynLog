...
public:
    ...
    void server_init()
    {
        listen_socaket_ = socket(AF_INET, SOCK_STREAM, 0);
        if (-1 == listen_socaket_)
        {
        ...
        if (-1 == bind(listen_socaket_, (struct sockaddr *)&any, sizeof(any)))
        {
            std::cout << __FILE__ << __LINE__ << "error: bind socket filed" << std::endl;
            return;
        }

        if (-1 == listen(listen_socaket_, backlog))
        {
    ...
    void server_start()
    {
        while (true)
        {
            ...
            int connfd = accept(listen_socaket_, (struct sockaddr *)&client_addr, &len);
            if (connfd < 0)
            {
            ...
            uint16_t client_port = ntohl(client_addr.sin_port);
